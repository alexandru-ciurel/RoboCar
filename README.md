...
A metal car chassis with 4 electrical motors that move the wheels - both motors on each side (left and right) being connected in series. As a logical view, this meant we only had to deal with 2 "motors"/objects in our code (left and right). These motors were connected to an H2 Bridge, which could control their states by adjusting 4 inputs (2 for each side, giving us the opportunity to have 4 states, but only 3 were needed: OFF=STOP, forward, backward). The inputs were controlled by Python code, using the GPIO library. Besides controlling the movement of the car, we've also used 3 light sensors and 1 ultrasound sensor which were directly connected to the Raspberry PI pins. They helped us get the much-needed information regarding the position/state and an overview of the car/system. Light sensors - for knowing were the car is in relation to the line we were trying to follow; Ultrasound sensor - for measuring the distance to the objects that were in front of our car (if the sensor reported a large distance or an error/no distance it meant that there wasn't any object in front of our car).
